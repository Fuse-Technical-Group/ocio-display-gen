...
def copy_samples(tmp_path: Path) -> Path:
    """Copy the shipped sample inputs into tmp_path; return decisions path."""
    (tmp_path / "measurements").mkdir()
    shutil.copy(REPO_DIR / "decisions.yaml", tmp_path / "decisions.yaml")
    shutil.copy(REPO_DIR / ARTIFACT_NAME, tmp_path / ARTIFACT_NAME)
    return tmp_path / "decisions.yaml"


def rewrite_pointer_sha256(decisions_path: Path, sha256: str) -> None:
    """Rewrite the promotion pointer's recorded digest in place."""
    data: dict[str, Any] = yaml.safe_load(decisions_path.read_text(encoding="utf-8"))
    data["measurements"]["sha256"] = sha256
    decisions_path.write_text(yaml.safe_dump(data), encoding="utf-8")


def generate_config_text(decisions_path: Path) -> str:
    """The library generation path: load, characterize, register, record."""
    decisions, measurements, provenance = load_inputs(str(decisions_path))
...
def test_description_carries_input_hashes_and_version(tmp_path: Path) -> None:
    ...
    config_text = generate_config_text(decisions_path)
...
def test_provenance_appends_to_base_description(tmp_path: Path) -> None:
    ...
    reloaded = OCIO.Config.CreateFromStream(generate_config_text(decisions_path))
...
def test_malformed_recorded_digest_refuses(tmp_path: Path, bad_digest: str) -> None:
    decisions_path = copy_samples(tmp_path)
    rewrite_pointer_sha256(decisions_path, bad_digest)
...
def test_uppercase_recorded_digest_accepted(tmp_path: Path) -> None:
    ...
    rewrite_pointer_sha256(
        decisions_path, sha256_file(tmp_path / ARTIFACT_NAME).upper()
    )
